    ...
    <nav v-if="!route.meta.minimal" class="fixed top-0 w-full z-50 bg-surface/80 backdrop-blur-md border-b border-outline-variant/30">
      <div class="max-w-container-max mx-auto px-margin-mobile h-16 flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
          <router-link :to="{ name: 'identification-report' }" class="font-label-caps text-xs tracking-wider text-secondary hover:text-primary transition-colors whitespace-nowrap">
            {{ $t('nav.searchReport') }}
          </router-link>
          <router-link :to="{ name: 'member-profile' }" class="font-label-caps text-xs tracking-wider text-secondary hover:text-primary transition-colors whitespace-nowrap">
            {{ $t('nav.memberCenter') }}
          </router-link>
